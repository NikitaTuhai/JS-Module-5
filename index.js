...
const getUsersByGender = (arr, gender) =>
  arr.filter(user => user.gender === gender).map(userName => userName.name);
...
const getTotalBalance = arr =>
  arr.map(user => user.balance).reduce((prev, next) => prev + next, 0);
...
//  */
const getUsersByFriend = (arr, name) => arr.filter(user => user.friends.includes(name)).map(userName => userName.name)
